...
--]]
--== Configuration;
local defaultDatabase = "https://nexusdata-ad7b5-default-rtdb.firebaseio.com/"; -- Set your database link
--For Ro-powers and Colo Ranks
local ExtraDataDatabase = ""
--Token table

local authenticationTokens = {
	['Reforged'] = "5R1CoYPNdZhGG01rCR3pOruwsCGjhteQm4gPBc5E", --Main Data
	['ExtraData'] = "" --Ro-Powers Data
...
function FirebaseService:GetFirebase(name, database)
	...
	local token = authenticationTokens[name]
	...
	**--]]
	...
	function Firebase:SetAsync(directory, value, header)
		if not UseFirebase then print("Cant Run") return end
		if value == "[]" then print("Bad Value found") self:RemoveAsync(directory); return end;
